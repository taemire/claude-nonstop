...
import { notifyDesktop, APP_LABEL } from './notify.js';
...
/**
 * Rate limit detection pattern (canonical form).
 ...
 * The qualifier between "your" and "limit" (session/usage/weekly) is optional.
 */
const RATE_LIMIT_PATTERN = /(?:Limit reached|You've hit your (?:[\w-]+ )?limit)\s*[·•]\s*resets\s+(.+?)(?:\s*$|\n)/im;

/**
 * Drift-tolerant rate-limit pattern (loose fallback).
 * Same shape as the canonical pattern — limit-phrase + separator +
 * recovery-verb + time-string — but permits cosmetic variation in
 * each piece so a renderer change on the upstream CLI doesn't blind
 * the runner. Used only when the canonical regex misses; the
 * structural requirement keeps false-positive risk low (a model
 * paraphrasing rate-limit info in prose is unlikely to emit all four
 * pieces in this exact order).
 *
 * Variants accepted:
 *   - limit phrase: "Limit reached", "You've hit your [session|usage|weekly]
 *     limit", "reached your … limit", "Rate limit (reached|hit)"
 *   - separator: any of `· • — - , : |` plus whitespace
 *   - recovery verb: "resets" / "resumes" / "available again" /
 *     "try again"
 *
 * Capture group 1 is the time string (same semantics as the canonical
 * pattern), so the existing reset-time consumer code works unchanged.
 */
const RATE_LIMIT_PATTERN_LOOSE =
  /(?:Limit reached|You've hit your (?:[\w-]+ )?limit|reached your (?:[\w-]+ )?limit|Rate limit (?:reached|hit))\s*[·•—\-,:|]+\s*(?:resets?|resumes?|available again|try again)\s+(.+?)(?:\s*$|\n)/im;

/**
 * Two-strategy rate-limit detection on the ANSI-stripped buffer.
 *   1. Canonical phrase — `RATE_LIMIT_PATTERN` against the stripped text.
 *   2. Loose fallback — `RATE_LIMIT_PATTERN_LOOSE` for renderer drift.
 *
 * @param {string} stripped - ANSI-stripped PTY accumulator.
 * @returns {{ matched: boolean, via: 'phrase'|'phrase-loose'|null, resetTime: string|null }}
 */
function detectRateLimit(stripped) {
  const strict = stripped.match(RATE_LIMIT_PATTERN);
  if (strict) {
    return { matched: true, via: 'phrase', resetTime: strict[1].trim() };
  }
  const loose = stripped.match(RATE_LIMIT_PATTERN_LOOSE);
  if (loose) {
    return { matched: true, via: 'phrase-loose', resetTime: loose[1].trim() };
  }
  return { matched: false, via: null, resetTime: null };
}
...
/**
 * Cooldown window (ms) for an account that just triggered a swap.
 * The just-failed account is excluded from candidate selection for this long
 * so the picker cannot bounce A→B→A→B when both accounts' server-side state
 * still rejects despite the local usage API showing headroom (observed
 * 2026-05-03: a freshly-reset 9% account immediately rate-limited after swap).
 * Cleared in bulk when a sleep-until-reset cycle completes.
 */
const FAIL_COOLDOWN_MS = 3 * 60 * 1000;

/**
 * Backoff inserted before restarting on the same account in the admin-disabled
 * false-positive branch. Keeps the loop from hot-spinning if the trigger
 * phrase recurs immediately on every relaunch (e.g., banner replay residue).
 */
...
export async function run(claudeArgs, selectedAccount, allAccounts, options = {}) {
  ...
  // Map<accountName, expiresAtMs>. Accounts here are excluded from candidate
  // selection until their cooldown expires — prevents same-pair ping-pong
  // when the local usage API is out of sync with the server's actual state.
  const recentlyFailed = new Map();
  const pruneCooldowns = () => {
    const now = Date.now();
    for (const [k, v] of recentlyFailed) if (v <= now) recentlyFailed.delete(k);
  };
  ...
  while (swapCount <= maxSwaps) {
    ...
    if (swapReason === 'admin-disabled' &&
      ...
      if (cred.token) {
        ...
        if (apiOk && belowThreshold) {
          ...
          // Backoff before relaunching: if the trigger phrase recurs
          // immediately every time (e.g., banner replay residue, sub-agent
          // stdout pollution) the false-positive branch can otherwise
          // hot-spin restart→detect→restart. Sleep is interruptible so
          // SIGINT still exits cleanly.
          const { interrupted: fpInterrupted } = await sleep(FALSE_POSITIVE_BACKOFF_MS);
          if (fpInterrupted) {
            console.error('\n[claude-nonstop] False-positive backoff interrupted by signal. Exiting.');
    ...
    // Cool down the just-failed account so the picker cannot bounce straight
    // back to it — prevents A→B→A→B ping-pong when both accounts' actual
    // server-side state still rejects despite the local usage API reporting
    // headroom. Manual swaps don't get a cooldown; they're an explicit user
    // override and the user may want to come straight back.
    if (swapReason === 'rate-limit' || swapReason === 'admin-disabled') {
      recentlyFailed.set(currentAccount.name, Date.now() + FAIL_COOLDOWN_MS);
    }
    pruneCooldowns();
    ...
    // When the swap budget is exhausted, fall through to the sleep-until-reset
    // branch instead of exiting. The hard-exit caused user-visible "complete
    // freeze" after a ping-pong burst (observed 2026-05-03: 9 swaps in 11 min
    // followed by exit 1, leaving the session dead even though accounts would
    // recover within the hour). The branch below sleeps until the earliest
    // reset, then resets the swap budget so the loop can resume.
    let exhaustionFallback = false;
    if (swapCount > maxSwaps) {
      console.error('[claude-nonstop] Maximum swap attempts reached — falling back to sleep-until-reset.');
    ...
    // Combine admin-disabled blocklist with the cooldown set so the picker
    // skips both. blocklist is a Set of names; we union it with cooldown keys.
    const cooldownNames = new Set(recentlyFailed.keys());
    const totalExclusions = new Set([
      ...(blocklist || []),
      ...cooldownNames,
    ]);
    let best = pickBestAccount(accountsWithUsage, currentAccount.name, { usePriority: hasPriorities, excludeNames: totalExclusions });

    // Sleep-until-reset triggers when:
    //   1. exhaustionFallback — swap budget already exceeded above.
    //   2. rate-limit + best candidate is at/above EXHAUSTION_THRESHOLD.
    //   3. no candidate available because the cooldown/blocklist consumed
    //      every account — without this we'd hit the bare "no alternative
    //      accounts available" exit and leave the session dead even when
    //      accounts would recover shortly.
    // Admin-disable alone (with a healthy candidate) skips the sleep — there
    // is no clock-based reset for blocklisted accounts.
    //
    ...
    if (triggerSleep) {
      // Sleep until whichever comes first: an account's reset, or a cooldown
      // expiry. Without including cooldowns, the runner would oversleep when
      // the only thing keeping all candidates out is a 3-minute cooldown.
      const earliestReset = findEarliestReset(accountsWithUsage);
      const earliestCooldownMs = recentlyFailed.size > 0
        ? Math.min(...Array.from(recentlyFailed.values())) - Date.now()
        : Infinity;
      ...
      if (sleepMs > 0) {
        ...
        notifyDesktop(
          `${APP_LABEL} · 절전`,
          `모든 계정 한도 임박 — ${formatDuration(clampedMs)} 절전 후 재개 (현재 "${currentAccount.name}")`,
        );
        if (remoteAccess) {
        ...
        // Cooldowns shorter than the sleep have expired; drop them so they
        // don't keep accounts excluded after the wake-up pick.
        pruneCooldowns();
        ...
        best = pickBestAccount(accountsWithUsage, undefined, { usePriority: hasPriorities, excludeNames: refreshedExclusions });

        notifyDesktop(
          `${APP_LABEL} · 재개`,
          best?.account?.name
            ? `절전 종료 — "${best.account.name}" 계정으로 재개합니다`
            : '절전 종료 — 계정 재확인 중...',
        );
        if (remoteAccess) {
        ...
        if (exhaustionFallback) {
          // Replenish the swap budget after a successful sleep so the loop
          // can resume. Subtract allAccounts.length so a fresh ping-pong
          // burst still trips the fallback again rather than running away.
          swapCount = Math.max(0, swapCount - allAccounts.length);
          exhaustionFallback = false;
        } else {
        ...
        // No reset time available and no cooldown to wait on — preserve the
        // pre-patch behavior: bail out rather than spin.
        console.error('[claude-nonstop] No reset times available; cannot recover.');
        process.exitCode = 1;
        return;
      }
    }
    ...
    // Desktop banner (local, always — independent of Slack/remote access) so
    // the user sees the switch even when running without the Slack subsystem.
    notifyDesktop(
      `${APP_LABEL} · 계정 전환`,
      `한도 도달: "${currentAccount.name}" → "${nextAccount.name}" (swap ${swapCount}/${maxSwaps})`,
    );
    ...
    // Update args for resume if we have a session — include a continuation
    // message that BOTH announces the switch (it appears in the resumed chat as
    // a user turn) and tells Claude to pick up where it left off.
    if (sessionId) {
      const switchMsg = `[claude-nonstop] 직전 계정이 사용량 한도에 도달하여 "${nextAccount.name}" 계정으로 자동 전환했습니다. 직전 작업을 이어서 계속해 주세요.`;
      claudeArgs = buildResumeArgs(claudeArgs, sessionId, switchMsg);
    }
...
function runOnce(claudeArgs, account, existingSessionId, options = {}) {
  return new Promise((resolve) => {
    ...
    child.onData((data) => {
      ...
      const rl = detectRateLimit(stripped);
      if (rl.matched) {
...
export {
  stripAnsi, normalizeBannerBuffer, detectAdminDisabled, detectRateLimit,
  extractResumeSessionId, buildResumeArgs, RATE_LIMIT_PATTERN, RATE_LIMIT_PATTERN_LOOSE,
