 ...
 * usage allocation; when they do, Claude Code surfaces:
 ...
 *
 * NOTE: The previous CTA-only alternative ("/extra-usage to request more
 * usage from your admin") was removed because it produced false positives
 * — that string is a generic CLI hint that can appear in documentation,
 * model output (when discussing this feature), pasted logs, etc., causing
 * spurious account blocklisting. Detection now requires the explicit
 * disabled-state sentence, which only the Claude CLI emits on real
...
export async function run(claudeArgs, selectedAccount, allAccounts, options = {}) {
  ...
  while (swapCount <= maxSwaps) {
    ...
    // sleep-until-reset thrash-avoidance branch below.
    const swapReason = result.rateLimitDetected
      ? 'rate-limit'
      : (result.adminDisableDetected ? 'admin-disabled' : null);
 ...
 *
 * @returns {Promise<{ exitCode: number|null, rateLimitDetected: boolean, adminDisableDetected: boolean, resetTime: string|null, sessionId: string|null }>}
 */
function runOnce(claudeArgs, account, existingSessionId, options = {}) {
  return new Promise((resolve) => {
    ...
    child.onData((data) => {
      ...
      if (rateLimitDetected || adminDisableDetected) return;
    ...
    for (const sig of signals) {
      const handler = () => {
        if (!rateLimitDetected && !adminDisableDetected) {
