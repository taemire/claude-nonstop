 ...
 * remove Unicode box-drawing glyphs (U+2500–U+257F), and collapse all
 * whitespace runs (incl. CRLF + box padding) into a single space.
 *
 * The genuine admin-disable banner from Claude Code wraps its sentence
 * across multiple PTY lines bounded by `│` glyphs. Without this
 * normalization, the literal sentence is not contiguous in the buffer
 * and a flat-text regex cannot match it.
 */
...
function runOnce(claudeArgs, account, existingSessionId, options = {}) {
  return new Promise((resolve) => {
    ...
    child.onData((data) => {
      ...
      if (!adminDetectDisabled && ADMIN_DISABLE_PATTERN.test(normalizeBannerBuffer(outputBuffer))) {
...
export {
  stripAnsi, normalizeBannerBuffer,
